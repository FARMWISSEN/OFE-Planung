...
CHANGED_FILES=0
PYTHON_FILES=`find ofe_datenfusion -regex ".*\(ui\|py\)$" -type f`
...
do
  TRANSLATION_FILE="ofe_datenfusion/i18n/$LOCALE.ts"
...
then
  ...
  for LOCALE in ${LOCALES}
  do
    echo "ofe_datenfusion/i18n/"${LOCALE}".ts"
    # Note we don't use pylupdate with qt .pro file approach as it is flakey
    # about what is made available.
    pylupdate4 -noobsolete ${PYTHON_FILES} -ts ofe_datenfusion/i18n/${LOCALE}.ts
  done
else
